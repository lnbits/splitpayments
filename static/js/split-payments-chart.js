  ...
  methods: {
    createFlowChart() {
      try {
        ...
        const targets = this.splitDiagramData.filter(
          item => item.type === 'target'
        )
        const sourceRemaining = this.splitDiagramData.filter(
        ...
        bottomRowItems.forEach((item, index) => {
          if (item.type === 'source_remaining') {
            ...
            const lineThickness = Math.max(
              3,
              (item.percent / maxPercent) * maxThickness
            )
    ...
    drawWalletIcon(
      ...
      if (type === 'target' || type === 'source_remaining') {
        ...
        percentText.setAttribute(
          'fill',
          type === 'source_remaining' ? '#96A6FF' : '#f59e0b'
        )
    ...
    drawBitcoinLogo(svg, x, y, isDarkTheme = false) {
      ...
      const incomingText = document.createElementNS(
        'http://www.w3.org/2000/svg',
        'text'
      )
